...
# Regressão Linear
X = filtered_data[['Kt']].values
X = np.hstack([np.ones((X.shape[0], 1)), X])  # Adiciona a coluna de 1s para o intercepto
y = filtered_data['Kd'].values

# Ajustar o modelo de regressão linear
beta = np.linalg.lstsq(X, y, rcond=None)[0]
beta_0, beta_1 = beta

# Calcular a curva da regressão linear
y_pred = X @ beta

# Cálculo dos intervalos de confiança para beta_0 e beta_1
alpha = 0.05
n = len(filtered_data)
MSE = np.sum((y - y_pred) ** 2) / (n - 2)
cov_matrix = MSE * np.linalg.inv(X.T @ X)
conf_intervals = np.sqrt(np.diag(cov_matrix)) * stats.t.ppf(1 - alpha / 2, df=n - 2)

# Dados para a tabela
resultados = pd.DataFrame({
    'Parâmetro': ['beta_0', 'beta_1', 'Intervalo de Confiança Beta_0', 'Intervalo de Confiança Beta_1'],
    'Valor': [round(beta_0, 3), round(beta_1, 3),  
              f"[{round(beta_0 - conf_intervals[0], 3)}, {round(beta_0 + conf_intervals[0], 3)}]", 
              f"[{round(beta_1 - conf_intervals[1], 3)}, {round(beta_1 + conf_intervals[1], 3)}]"]
})
...
kt_range = np.linspace(0, 1, 100)
kd_linear_values = beta_0 + beta_1 * kt_range
plt.plot(kt_range, kd_linear_values, color='yellow', linestyle='--', label='Regressão Linear')
